...
def get_anytime_random_2d_env_configs(root_dir='.'):
    '''
    The anytime random 2d world dataset has 4 pairs of start and goal for each obstacle map.
    We transform one obstacle map into 4 environments for evaluation.
    '''
    with open(join("anytime_data", "anytime_random_2d", "test", "envs.json"), 'r') as f:
        anytime_random_2d_map_list = json.load(f)
    env_config_list = []
    for map_idx, env_dict_per_map in enumerate(anytime_random_2d_map_list):
        for start_goal_pair_idx in range(len(env_dict_per_map['start'])):
            env_config = {}
            env_config['img_idx'] = map_idx
            env_config['start_goal_idx'] = start_goal_pair_idx
...
def get_anytime_random_2d_problem_input(anytime_random_2d_env_config):
    '''
    The last None is to match outputs of other get_problem_input functions.
    '''
    env_img = cv2.imread(join("anytime_data", "anytime_random_2d", "test", "env_imgs", "{0}.png".format(anytime_random_2d_env_config['img_idx'])))
    binary_mask = get_binary_mask(env_img)
    env_dict = anytime_random_2d_env_config['env_dict']
    x_start = tuple(env_dict['start'][0])
    x_goal = tuple(env_dict['goal'][0])
...
def get_block_anytime_env_configs(root_dir='.'):
    """
    从 anytime_test_datasets/block_gap/block/envs.json 读取环境，
    生成与 get_random_2d_env_configs 一致的 env_config_list。
    每个环境只有一个 start/goal。
    """
    with open(join(root_dir, 'anytime_test_datasets', 'block_gap', 'block', 'envs.json'), 'r') as f:
        env_list = json.load(f)
    env_config_list = []
    for map_idx, env_dict_per_map in enumerate(env_list):
        env_config = {}
        env_config['img_idx'] = map_idx
        env_config['start_goal_idx'] = 0
        env_config['env_dict'] = copy(env_dict_per_map)
        # 保证只有一个起止对
        env_config['env_dict']['start'] = [env_dict_per_map['start'][0]]
        env_config['env_dict']['goal'] = [env_dict_per_map['goal'][0]]
        env_config_list.append(env_config)
    return env_config_list


def get_block_anytime_problem_input(env_config, root_dir='.'):
    """
    读取 anytime_test_datasets/block_gap/block/env_imgs/{img_idx}.png，
    重建 binary_mask，并返回 problem 字典。
    """
    env_img = cv2.imread(join(root_dir, 'anytime_test_datasets', 'block_gap', 'block', 'env_imgs', f"{env_config['img_idx']}.png"))
    binary_mask = get_binary_mask(env_img)
...
def get_gap_anytime_env_configs(root_dir='.'):
    """
    从 anytime_test_datasets/block_gap/gap/envs.json 读取环境。
    """
    with open(join(root_dir, 'anytime_test_datasets', 'block_gap', 'gap', 'envs.json'), 'r') as f:
        env_list = json.load(f)
    env_config_list = []
    for map_idx, env_dict_per_map in enumerate(env_list):
        env_config = {}
...
def get_gap_anytime_problem_input(env_config, root_dir='.'):
    """
    读取 anytime_test_datasets/block_gap/gap/env_imgs/{img_idx}.png，
    重建 binary_mask，并返回 problem 字典。
    """
    env_img = cv2.imread(join(root_dir, 'anytime_test_datasets', 'block_gap', 'gap', 'env_imgs', f"{env_config['img_idx']}.png"))
    binary_mask = get_binary_mask(env_img)
    env_dict = env_config['env_dict']
    x_start = tuple(env_dict['start'][0])
    x_goal = tuple(env_dict['goal'][0])
