...
class RRTStarVisualizer:
    def __init__(self, x_start, x_goal, env):
        ...
        self.scatter_pred1 = None
        self.scatter_pred2 = None
    ...
    def plot_grid(self, figure_title):
        ...
        # plt.axis("equal")
        self.ax.set_xlim(0, 224)
        self.ax.set_ylim(0, 224)
        self.agent_viz = plt.plot(self.x_start[0], self.x_start[1], c='r', marker='*', zorder=10, markersize=8)[0]  
        plt.scatter(self.x_goal[0], self.x_goal[1], s=50, c='y', marker='*', zorder=10)
...
class NIRRTStarVisualizer(IRRTStarVisualizer):
    ...
    def animation(self, vertices, vertex_parents, path, figure_title,\
        ...
        # self.plot_visited(vertices, vertex_parents, animation=False)
        if self.path_point_cloud_pred is not None:
            self.scatter_pred1 = plt.scatter(self.path_point_cloud_pred[:,0], self.path_point_cloud_pred[:,1], s=2, c='C1')  # 橙色的散点，表示预测的点（guidance state）
        if self.path_point_cloud_other is not None:
            self.scatter_pred2 = plt.scatter(self.path_point_cloud_other[:,0], self.path_point_cloud_other[:,1], s=2, c='C0')  # 蓝色的散点，表示其他预测的点（非guidance state）
        if c_best != np.inf:
            self.draw_ellipse(x_center, c_best, dist, theta)
        self.plot_path(path)
        if img_filename is None:
